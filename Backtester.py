import pandas as pd
import ChasingTheStatus as template  # Replace 'filename' with whatever your algorithm file is named
from copy import deepcopy


DATA_LOCATION = "./QFINdata"  # Set this to the location of your data folder

# List of stock names (without .csv)
products = ["UEC", "SOBER", "SMIF", "FAWA"]  # This list determiens the stocks your algorithm will be backtested on
...
n_timestamps = min(len(price_series[products[0]]), len(price_series[products[1]]), len(price_series[products[2]]), len(price_series[products[3]]))
n_timestampsTrain = n_timestamps *0.6
n_timestampsVal = n_timestamps *0.8
...
for i in range(n_timestamps):
    ...
    for product in products:
        current_data[product] = {"Timestamp": i, "Bid": price_series[product].iloc[i]["Bids"],
    ...
    for product in order:
        ...
        if quant > 0:  # Team is buying
            ...
            if positions[product] + quant > position_limit:
                ...
                quant = 0
                #print("At time ", i , " upper limit reached for ", product)
            ...
            if positions[product] + quant < -position_limit:
                ...
                quant = 0
                #print("At time ", i, " lower limit reached for ", product)
            # Change cash for this product
